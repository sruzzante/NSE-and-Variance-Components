...
# remove stations already in HYSETS
...
# stns$region<-substr(stns$gauge_id,1,2)
...
for(it in 1:nrow(stns)){
  ...
  dat<-read.delim(paste0("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/camels-us/basin_dataset_public_v1p2/usgs_streamflow/",
                         stns$huc_02[it],"/",stns$gauge_id[it],"_streamflow_qc.txt"),
                  sep = "",
                  header= FALSE,
                  col.names = c("gauge_id","year","month","day","q","flag"))%>%
...
write.csv(stn_var,"2.data/varComponents/camels_US.csv",row.names = FALSE)
...
st_write(stns,"2.data/GOF/camels_US.gpkg",append = FALSE)
