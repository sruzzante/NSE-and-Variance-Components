...
stns<-read.csv("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/caravan_ext_israel/attributes/il/attributes_other_il.csv")%>%
...
for(it in 1:nrow(stns)){
  ...
  nc_dat<-nc_open(paste0("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/caravan_ext_israel/timeseries/netcdf/il/",
                         stns$gauge_id[it],".nc"))
...
write.csv(stn_var,"2.data/varComponents/camels_il.csv",row.names = FALSE)
...
st_write(stns,"2.data/GOF/camels_il.gpkg",append = FALSE)
