...
stns<-read.csv("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/camels-cl/CAMELS_CL_v202201/catchment_attributes.csv",
               quote = '')
names(stns)<-substr(names(stns),3,1000)
names(stns)<-str_remove(names(stns),"\\.")
...
for(it in 1:nrow(stns)){
  ...
  dat<-streamflow_dat%>%
    select(year,date,all_of(paste0("X",stns$gauge_id[it])))%>%
    mutate(dt = ymd(date),
...
write.csv(stn_var,"2.data/varComponents/camels_cl.csv",row.names = FALSE)
...
st_write(stns,"2.data/GOF/camels_cl.gpkg",append = FALSE)
