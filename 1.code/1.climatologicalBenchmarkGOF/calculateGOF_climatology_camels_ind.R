...
# stns<-read.csv("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/caravan/Caravan/attributes/camels/attributes_caravan_camels.csv")
stns<-st_read("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/camels-ind/shapefiles_catchment/merged/all_gauge_stations.shp")%>%
  st_make_valid()%>%
  st_centroid()
...
for(it in 1:nrow(stns)){
  ...
  # nc<-nc_open(paste0("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/caravan/Caravan/timeseries/netcdf/camelscl/",
  #                    stns$gauge_id[it],".nc"))
  ...
  q<-streamflow[,paste0("X",as.numeric(stns$gauge_id[it]))]
  dt<-streamflow$dt
  # nc_close(nc)
  dat<-data.frame(dt,q)%>%
    mutate(year = year(dt),
...
write.csv(stn_var,"2.data/varComponents/camels_ind.csv",row.names = FALSE)
...
stns$region<-substr(stns$gauge_id,8,9)
stns%>%
  group_by(region)%>%
  summarize(across(NSE:KGE,~mean(.x,na.rm = TRUE)))
tmap_mode("view")
tm_shape(stns)+tm_dots(col = "NSE",breaks = c(-Inf,seq(-1,1,0.25)))



st_write(stns,"2.data/GOF/camels_ind.gpkg",append = FALSE)
