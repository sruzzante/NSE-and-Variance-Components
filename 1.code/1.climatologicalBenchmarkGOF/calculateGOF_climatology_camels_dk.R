...
stn_var<-data.frame(catch_id = stns$catch_id,
                    varSeas_stl = NA,
...
for(it in 1:nrow(stns)){
  ...
  # read in data
  dat<-read.csv(paste0("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/camels-dk/Dynamics/Gauged_catchments/CAMELS_DK_obs_based_",stns$catch_id[it],".csv"))%>%
    select(time,Qobs)%>%
    mutate(dt = ymd(time),
...
write.csv(stn_var,"2.data/varComponents/camels_dk.csv",row.names = FALSE)
...
st_write(stns,"2.data/GOF/camels_dk.gpkg",append = FALSE)
