...
watersheds<-st_read("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/GRDC/shapefiles/2025-02-11 13-54-59.shp")%>%
  st_transform(st_crs("EPSG:4326"))

GOF_stns<-list.files("2.data/GOF/",pattern = "GRDC",full.names = TRUE)%>%
  lapply(st_read)%>%
  bind_rows ()
watersheds<-watersheds%>%filter(grdc_no %in% GOF_stns$grdc_no)

watersheds$gauge_id<-watersheds$grdc_no
...
write.csv(dat,"2.data/worldclim/grdc_clim.csv")
