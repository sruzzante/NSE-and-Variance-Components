...
watersheds<-st_read("../DATA/1.Spatial_data/global/sw_surfacewater_streamflow_runoff_river_network_waterstress/lamah-ce/A_basins_total_upstrm/3_shapefiles/Basins_A.shp")%>%
  st_transform(st_crs("EPSG:4326"))%>%
  left_join(stns%>%st_drop_geometry()%>%select(ID,country,govnr))%>%
  st_make_valid() 

watersheds$gauge_id<-watersheds$govnr
...
write.csv(dat,"2.data/worldclim/lamah_ce_clim.csv")
