...
# Evaluate LSTM model from Kraft (2025)
...
# Gauge-based simulations were shared privately via email
...
for(it in 1:nrow(stns)){
  ...
  dat<-data.frame(
    ...
    QSim =  ncvar_get(dat_nc,"Qmm_mod",start = c(1,it),count = c(22645,1) )
  )
...
write.csv(stns,"2.data/highLowBenchmarkGOF/GOF_Kraft_lstm.csv")
